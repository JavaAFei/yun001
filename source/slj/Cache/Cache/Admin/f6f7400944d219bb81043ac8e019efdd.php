...
//--></script></head><body width="100%"><div id="loader" ><?php echo L('load_page');?></div><div id="result" class="result none"></div><div class="mainbox"><?php if(empty($_GET['isajax'])): ?><div id="nav" class="mainnav_title"><div id="lang"><?php if(APP_LANG): parse_str($_SERVER['QUERY_STRING'],$urlarr); unset($urlarr[l]); $url='index.php?'.http_build_query($urlarr); if(is_array($Lang)): $i = 0; $__LIST__ = $Lang;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$langvo): $mod = ($i % 2 );++$i;?><a href="<?php echo ($url); ?>&l=<?php echo ($langvo["mark"]); ?>" <?php if($langvo[mark]==$_SESSION['YP_lang']): ?>class="on"<?php endif; ?>><?php echo ($langvo["name"]); ?></a><?php endforeach; endif; else: echo "" ;endif; endif; ?></div><ul><a href="<?php echo U($nav[bnav][model].'/'.$nav[bnav][action],$nav[bnav][data]);?>"><?php echo ($nav["bnav"]["name"]); ?></a>|
	...
	</script><?php endif; ?><form name="myform"  id="myform"  action="<?php echo U($module_name.'/edit');?>" method="post"><table cellpadding=0 cellspacing=0 class="table_form"width="100%"><tr><td width="120"><?php echo L('filename');?></td><td><input type="text"   class="input-text"  validate="required:true, minlength:2, maxlength:30"   name="file" value="<?php echo ($filename); ?>" <?php if($action_name=='edit'): ?>readonly="readonly"<?php endif; ?>/><?php if($action_name=='add'): ?><input type="radio" name="type" value="html" checked /><?php echo L(file_type_html);?><input type="radio"  name="type" value="css" /><?php echo L(file_type_css);?><input type="radio"  name="type" value="js"/><?php echo L(file_type_js); endif; ?></td></tr><tr><td><?php echo L('content');?></td><td><textarea id="content"  name="content"  style="width:95%; height:400px" validate="required:true, minlength:4" ></textarea></td></tr></table><div class="btn"><INPUT TYPE="submit"  name="dosubmit" value="<?php echo L('dosubmit');?>" class="button" ><input TYPE="reset"  value="<?php echo L('cancel');?>" class="button"></div></form></body></html>